...
interface ActivityForm extends ActivityPayload {
  id: number | null
}
type Filters = { keyword: string; dateRange: string[] | null }

const form = ref<ActivityPayload>({ title: '', tag: '', content: '', pinned: false })
...
const editForm = ref<ActivityForm>({ id: null, title: '', tag: '', content: '', pinned: false })
...
async function loadList() {
  ...
  try {
    ...
    const data = await http.get<PageResult<Activity>>('/activities', {
      page: page.value - 1,
      size: pageSize,
    ...
    ElMessage.error(errorMessage(err, '加载活动失败'))
  } finally {
    loading.value = false
  }
}
...
async function publish() {
  ...
  try {
    ...
    ElMessage.error(errorMessage(err, '新增失败'))
  } finally {
...
@media (max-width: 768px) {
  ...
  .activity-manage :deep(.el-card__body) {
    padding: 12px;
  }
}
.form-card {
