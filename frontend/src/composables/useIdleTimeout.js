 ...
 * @param {number}   [options.timeout=1800000]  空闲阈值（毫秒），默认 30 分钟
 * @param {number}   [options.checkInterval=10000] 兜底巡检间隔（毫秒）
 * @param {string[]} [options.events]           参与计数的交互事件
 * @param {Function} [options.isEnabled]        空闲判定是否生效，默认恒为 true
 * @param {Function} [options.onTimeout]        判定为空闲后的回调
 */
